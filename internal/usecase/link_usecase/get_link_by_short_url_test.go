...
func TestGetLinkByShortURLUseCase(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			before: func(f fields, args args) {
				...
				f.linkRepo.EXPECT().GetLinkByShortURL(args.shortURL).Return(link, nil)
			},
		},
		{
			name: "error get link by short url",
			args: args{
				shortURL: "https://myDomain.ru/Ur3j9E5tAd",
			},
			wantErr: errTest,
			before: func(f fields, args args) {
				f.linkRepo.EXPECT().GetLinkByShortURL(args.shortURL).Return(nil, errTest)
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			//выполнили
			e, err := uc.GetLinkByShortURL(tt.args.shortURL)
